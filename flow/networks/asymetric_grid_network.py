...
class TrafficLightAsymetricGridNetwork(Network):
    ...
    * **grid_array** : dictionary of grid array data, with the following keys
      ...
      * **row_inner_lengths** : length of inner edges in traffic light grid network
      * **cols_inner_lengths** : length of inner edges in traffic light grid network
    ...
    * **speed_limit** : speed limit for all edges. This may be represented as a
    ...
    >>>                 'row_num': 3,
    >>>                 'col_num': 2,
    >>>                 'row_inner_lengths': [500,200,300],
    >>>                 'col_inner_lengths': [250,150,600],
    >>>                 'short_length': 500,
    >>>                 'long_length': 500,
    >>>                 'cars_top': 20,
    >>>                 'cars_bot': 20,
    ...
    def __init__(
        ...
        self.row_inner_lengths = self.grid_array["row_inner_lengths"]
        self.col_inner_lengths = self.grid_array["col_inner_lengths"]
        self.row_num = len(self.row_inner_lengths) + 1
        self.col_num = len(self.col_inner_lengths) + 1

        self.short_length = self.grid_array["short_length"]
        self.long_length = self.grid_array["long_length"]
        if self.short_length != self.long_length:
            raise Exception(
                "It might be bugged with short_length != long_lenght ({}!={})".format(self.short_length, self.long_length))
        ...
        self.use_traffic_lights = net_params.additional_params.get("traffic_lights", True)

        # radius of the inner nodes (ie of the intersections)
        self.inner_nodes_radius = 2.9 + 3.3 * max(self.vertical_lanes, self.horizontal_lanes)

        # total number of edges in the network
        self.num_edges = 4 * ((self.col_num + 1) * self.row_num + self.col_num)

        # name of the network (DO NOT CHANGE)
        self.name = "IChangeItIfIWantTo"
    ...
    def _inner_nodes(self):
        ...
        xs = [0]
        for x in self.col_inner_lengths:
            xs.append(x + xs[-1])
        ys = [0]
        for x in self.row_inner_lengths:
            ys.append(x + ys[-1])
    ...
    def _outer_nodes(self):
        ...
        for col in range(self.col_num):
            x = xs[col]  # col * self.inner_length
            y = ys[-1]  # (self.row_num - 1) * self.inner_length
        ...
        for row in range(self.row_num):
            x = xs[-1]  # (self.col_num - 1) * self.inner_length
            y = ys[row]  # row * self.inner_length
    ...
    def _inner_edges(self):
        ...
        def new_edge(index, from_node, to_node, orientation, lane, lenght):
            return [{
                "id": lane + index,
                "type": orientation,
                "priority": 78,
        ...
        x_len = copy(self.row_inner_lengths)
        y_len = copy(self.col_inner_lengths)
        # print("x_len",x_len)
        # print("y_len",y_len)
        # print("self.row_num",self.row_num)
        # print("self.col_num",self.col_num)
        # Build the horizontal inner edges
        for i in range(self.row_num):
            for j in range(self.col_num - 1):
                node_index = i * self.col_num + j
                index = "{}_{}".format(i, j + 1)
                edges += new_edge(index, node_index + 1, node_index, "horizontal", "top", y_len[j])
                edges += new_edge(index, node_index, node_index + 1, "horizontal", "bot", y_len[j])
        ...
        for i in range(self.row_num - 1):
            for j in range(self.col_num):
                ...
                edges += new_edge(index, node_index, node_index + self.col_num, "vertical", "right", x_len[i])
                edges += new_edge(index, node_index + self.col_num, node_index, "vertical", "left", x_len[i])
        return edges
    ...
    def specify_edge_starts(self):
        # raise Exception("We dont use that")
        """See parent class."""
        edgestarts = []
    ...
    def gen_custom_start_pos(cls, net_params, initial_config, num_vehicles):
        ...
        row_num = len(grid_array["row_inner_lengths"]) + 1
        col_num = len(grid_array["col_inner_lengths"]) + 1
