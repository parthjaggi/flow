...
"""--------------------------------------------AUTOMATIC PLOTTING OF TIME SPACE DIAGRAMS---------------------------------------------------""" 
...
if __name__ == '__main__':
    ...
    path = "./flow/examples/sumo/OVM_PNS"                                                    #Change this 
    ...
    j=0 
    for dir_i in files:
        # from the name, extract the controller name 
        #controller_one = dir_i[1]+dir_i[2]+dir_i[3] 
        #controller_two = dir_i[6]+dir_i[7]
       
        # from the name, extract number of cars (x)
        #y = dir_i.split('I')                                                                 #Change this ( I for IDM, O) for OVM 
        y = dir_i.split('O')
        x =  int(y[0])

        # check for the elements in the dir_i folder and choose the file that ends with .csv
        m = glob.glob("flow/examples/sumo/OVM_PNS/{}OVM_{}PNS/*.csv".format(x,22-x))     #Change this
        file_path = m[0]
        ...
        pos, speed, time = get_time_space_data(emission_data,total_len=260)
        ...
        # choose a title
        title = '{}OVM,{}PNS'.format(x,22-x)                         #Change this
        ...
        # save image
        plt.savefig('{}OVM,{}PNS'.format(x,22-x))                #Change this
        j+=1    
        print('{} left'.format(22-j)) 
